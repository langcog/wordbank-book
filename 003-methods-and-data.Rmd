# Methods and Data^[Some material in this chapter is adapted from @frank2016.] {#methods-and-data}
...
Table \@ref(tab:methods-overview) shows an overview of the instruments in Wordbank.

```{r methods-overview, results="asis"}
cat(dt_caption("Overview of the available instruments in the Wordbank dataset."))
items %>%
  ...
  select(language, form, categories, items, words, age_min, age_max) %>%
  dt()
```

The number of administrations available is highly variable across instruments and languages, however. Figure \@ref(fig:methods-admins) shows the distribution of administrations across forms and languages.

```{r methods-admins, fig.cap="Log-scaled number of administrations for each instrument."}
...
These instruments have global reach, although the maximal number cover North America and Western Europe. African, South American, and South/South-East Asian languages are notably under-represented. Figure \@ref(fig:methods-map) indicates which countries' languages are represented.

```{r methods-map, fig.cap="World map with countries shaded whose dominant language is represented in the dataset.", fig.height=3.6}
...
```{r methods-longitudinal, fig.cap="Number of children for whom there are multiple administrations for each instrument, split into bins."}
longitudinal <- admins %>%
...
```{r methods-russian-animals, fig.cap="Mean proportion children reported to produce each item in the Animal Sounds category for Russian Words and Sentences."}
...
<<<<<<< HEAD:003-methods-and-data.Rmd
In our first example, Mandarin Words & Sentences data are reported by @tardif2009 in a study of both Mandarin- and Cantonese-learning children. The data reported there show a pronounced Mandarin advantage. As it turns out, this advantage is almost unprecedented relative to other languages. We plot the median production for 24-month olds in Figure \@ref(fig:vocab-ords-mandarin). This figure reveals both how large the Mandarin advantage and the high level of vocabulary reported for Hebrew speakers as well, which is less unusual in raw scores because of the relatively smaller number of items on the Hebrew form. 
=======
Looking first at Mandarin Words & Sentences, these data are reported by @tardif2009 in a study of both Mandarin- and Cantonese-learning children. The data reported there show a pronounced Mandarin advantage. As it turns out, this advantage is almost unprecedented relative to other languages. We plot the median production for 24-month olds in Figure \@ref(fig:vocab-ords-mandarin). This figure reveals both how large the Mandarin advantage and the high level of vocabulary reported for Hebrew speakers as well, which is less unusual in raw scores because of the relatively smaller number of items on the Hebrew form. 
>>>>>>> 593a0a58930a73aff88709fbb69b962e22080e87:003-methods.Rmd
