...
include_graphics("tex/parent_model.png", auto_pdf = FALSE)
```

```{r psycho-parent_sketch, fig.cap="The intuitive structure of parent report."}
include_graphics("images/parent_report_sketch.png")
```
...
```{r psycho-get_data_ws, eval=FALSE}
...
eng_ws <- eng_ws %>%
  select(data_id, value, age, production, sex, definition, category)

# write_feather(eng_ws, "data/psychometrics/eng_ws_raw_data.feather")
save(eng_ws, file = "data/psychometrics/eng_ws_raw_data.Rds")
```
...
mod_4pl <- mirt::mirt(d_mat_ws, 1, itemtype='4PL', verbose=TRUE)
...
save(file = "data/psychometrics/eng_ws_mods_4pl.Rds", "mod_4pl","fscores_4pl", "coefs_4pl")
```
...
```{r psycho-get_data_wg, eval=FALSE}
eng_wg <- get_instrument_data(instrument_language = "English (American)",
                           instrument_form = "WG", 
                           iteminfo = TRUE,
                           administrations = TRUE)
...
# write_feather(eng_wg, "data/psychometrics/eng_wg_raw_data.feather")
save(eng_wg, file = "data/psychometrics/eng_wg_raw_data.Rds")
```

```{r psycho-prod_comp}
# eng_wg <- read_feather("data/psychometrics/eng_wg_raw_data.feather")
...
mod_4pl_wg_produces <- mirt::mirt(d_mat_wg_produces, 1, itemtype='4PL', verbose=TRUE)
mod_4pl_wg_understands <- mirt::mirt(d_mat_wg_understands, 1, itemtype='4PL', verbose=TRUE)
...
DT::datatable(longitudinal_admins %>% 
            group_by(langform) %>%
            count %>% 
            arrange(desc(nn)))
```
...
mirt_params <- function(dx) {
  ...
  mod_4pl <- mirt::mirt(d_lang_mat, 1, itemtype='4PL', verbose=TRUE)
