...
```{r psycho-items-ws, fig.cap="FIXME"}
ggplot(coefs_4pl,  
       aes(x = a1, y = d)) + 
  geom_point(alpha = .3) + 
...
```{r psycho-bounds-plot, fig.cap="FIXME"}
ggplot(coefs_4pl,  
       aes(x = g, y = u)) + 
  geom_point(alpha = .3) + 
  ggrepel::geom_text_repel(data = filter(coefs_4pl, 
...
```{r psycho-load-irt-data-wg, fig.cap="FIXME"}
...
```{r psycho-lexcat, fig.height=6.5, fig.cap="FIXME"}
coefs_4pl <- coefs_4pl %>%
  left_join(items %>% 
...
```{r psycho-lexcat-summary, fig.width=4.5, fig.cap="FIXME"}
