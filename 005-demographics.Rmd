...
get_demo_preds <- function (models, data, demo = ~sex) {
  ...
  data %>%
    ...
    separate(langform, into = c("language","form"), sep = "\\.") 
}
...
get_demo_model_summary <- function (models, level = "Male") {
  ...
  models %>%
    ...
    separate(langforms, into = c("language","form"), sep = "\\.") %>%
...
get_mmad_ratio <- function(data, measure = quo(production), demo = quo(sex), 
  ...
  data %>%
    ...
    filter(any(!! get_expr(demo) == high_level), 
           any(!! get_expr(demo) == low_level)) %>% # one of each necessary level
...
ggplot(analyses$WG.comprehension$sex$means,
  ...
  facet_wrap(~interaction(language, form)) +
...
ggplot(analyses$WG.comprehension$sex$mmad_ratio,
       aes(x = age, y = mmad, col = interaction(language, form))) +
  geom_jitter() +
...
b <- ggplot(analyses$WG.production$sex$mmad_summary, aes(x = interaction(language, form), y = mmad_mean)) + 
  geom_point() + 
...
b <- ggplot(analyses$WS.production$sex$mmad_summary, 
            aes(x = interaction(language, form),
                y = mmad_mean)) + 
  geom_point() + 
  # geom_pointrange(aes(ymin = mmad_ci_lower, ymax = mmad_ci_upper)) + 
  # ylim(0,1.5) + 
