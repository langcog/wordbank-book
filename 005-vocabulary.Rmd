...
highlight_labels <- highlight_data %>%
  filter(language %in% highlight_langs) %>%
  group_by(language) %>%
  filter(age == max(age)) %>%
  ungroup()
...
ggplot(highlight_data, 
  ...
  geom_text(aes(label = language), hjust = 1, nudge_y = -0.15, family = .font, size = lang_label_size,
            show.legend = FALSE, data = highlight_labels %>% slice(1)) +
  geom_text(aes(label = language), hjust = 1, nudge_y = 0.1, family = .font, size = lang_label_size,
            show.legend = FALSE, data = highlight_labels %>% slice(2)) +
  xlab("Age (months)") + 
...
select_median_labels <- select_medians %>%
  filter(age == 30) %>%
  ungroup()
...
ggplot(select_medians, 
  ...
  geom_smooth(se=FALSE, span = 1) +
  geom_text(aes(label = language), hjust = 1, nudge_y = -0.05, family = .font, size = lang_label_size,
            show.legend = FALSE, data = select_median_labels %>% slice(1)) +
  geom_text(aes(label = language), hjust = 1, nudge_y = 0.1, family = .font, size = lang_label_size,
            show.legend = FALSE, data = select_median_labels %>% slice(2)) +
  theme(legend.position = "bottom") + 
  scale_colour_manual(values = lang_colours, name = "") +
...
3. Differences could also be due to demographic differences across samples. We have examined sample composition in Chapter \@ref(methods-and-data) and see that -- to the extent we have access to demographic data -- sample composition does vary in features that affect vocabulary (e.g., maternal education, birth order; see Chapter \@ref(demographics) for fuller analysis). We are not yet in a position to conduct a full analysis of these differences, controlling for demographics, as data are sparse and the nature and extent of demographic differences *also* vary across cultures. Nevertheless, we can note, for example, that differences between Danish and English (American) children look quite similar (though noisier) in female, first-born children of college-educated mothers (Figure \@ref(fig:vocab-select-medians-DE)). This would suggest that demographic differences cannot fully explain the cross-linguistic differences that are observed. 

4. Differences could be due to cohort effects, in which older sets of data show differences from newer datasets. Most of our data date from the period 2005-2015, but some of our English and Spanish data are older, as they date to the period in which CDI forms were first being designed [the early 1990s; @fenson1994]. Unfortunately, we do not have reliable information about the collection date for many datasets, so we cannot use this variable in regression analyses. Naively, we would expect later cohorts to show higher vocabularies, consistent with the Flynn effect [@flynn1987]. Yet, the Danish data, for example are relatively recent and were collected online using standardized instructions. Danish is subject to its own issues (see below), but the Norwegian data are also relatively recent and are quite comparable to the English data. 
...
We next turn from central tendencies in early vocabulary to the issue of variability, one of the most important features of early vocabulary development [@fenson1994]. Across every language in the database there is huge variability in the vocabulary sizes reported for individual children, even within an age group. How can we characterize this variability? That is the question addressed by our next set of analyses. 
...
As an example, we zoom in on the English (American) production data from the Words & Sentences form. The canonical view of these data is given in Figure \@ref(fig:vocab-eng-prod). It is very clear that variability is the norm! There are children clsose to the floor and ceiling of the instrument at almost all age groups. 
...
We zoom in even further to consider just a single age group, 24-month-olds. A histogram of production vocabulary for this group is shown in Figure \@ref(fig:vocab-histogram). The distribution of vocabularies across children is far from normal, with many children at the very bottom of the scale and almost as many at the top. In other words, quite a few two-year-olds on their second birthday are reported to produce only a handful of words and others are producing nearly all of the 680 listed on the form. 
...
\noindent This statistic allows us to compare variability across measurements with different scales, an important concern when we want to compare forms with very different numbers of vocabulary items. For example, for two-year-olds, the mean productive vocabulary is `r roundp(mean(eng_prod_data$production[eng_prod_data$age == 24]), 0)` words, and the standard deviation is `r roundp(sd(eng_prod_data$production[eng_prod_data$age == 24]), 0)` words, leading to a CV of `r roundp(sd(eng_prod_data$production[eng_prod_data$age == 24])/mean(eng_prod_data$production[eng_prod_data$age == 24]))`. 
...
Appendix \@ref(appendix-variability) demonstrates that, although MADM is more appropriate for our data, CV and MADM are very highly correlated with one another across CDI datasets. 
...
ggplot(filter(madm_data, language == "English (American)"), 
  ...
  geom_smooth(aes(group = 1), colour = .pal()(1)) + 
  ylim(0,2) +
  xlim(8,30) + 
  ylab("MADM (MAD / median)") + 
  xlab("Age (months)") + 
  # .scale_colour_discrete(name = "Form") +
  scale_colour_manual(values = .pal()(3)[2:3], name = "Form") +
...
To get a sense of this variability, it can help to have a smaller dataset to consider. Imagine groups of three children. A group where one produced 30 words, one produced 100, and another produced 170 would have a MADM of `r roundp(madm(c(33,100,180)))`. In contrast, one where they were more closely grouped -- say 70, 100, 130 -- would have a MADM of `r roundp(madm(c(70,100,130)))`.
...
Does the general level of variability observed in American English hold for other languages? Figure \@ref(fig:vocab-madm2) shows the MADM for production across languages and instruments. This similarity in variability is quite striking. Between the first and second birthdays, children's early language is remarkably variable, but this variability itself is quite consistent across languages.
...
The same analysis for comprehension data is shown in Figure \@ref(fig:vocab-madm-comprehension). For comprehension, we see a gradual decrease in variability throughout development. The intercept for the 12-18 month period appears to be lower than that observed in production, despite -- or perhaps due to -- higher comprehension scores. This observation matches one made by @mayor2014, namely that, across children, production vocabulary appears more idiosyncratic than comprehension vocabulary. One speculative explanation for this difference would be the tremendous differences in speech-motor development (as well as general differences in loquacity) between toddlers [for an example, see @clark1993]. This variability would then carry over into production vocabulary size. Another possibility, however, is that true variability is masked by the overall lower reliability of comprehension items (see Chapter \@ref(psychometrics)). Our data do not allow us to distinguish between these two explanations.
...
The final plot in this sequence is shown in Figure \@ref(fig:vocab-madm-summary-comprehension), which shows 12-18-month average comprehension MADM values. These are slightly lower and slightly more variable than the production values shown above, but still display a quite consistent level of variability. 
...
get_mirt_params <- function(dx) {
  ...
  d <- lang_data %>%
    filter(!is.na(lexical_class)) %>% # get only words
  ...
  d_mat <- d_mat[,colMeans(d_mat) > 0.01 & colMeans(d_mat) < .99]
  mod_2pl <- mirt::mirt(d_mat, 1, itemtype='2PL', verbose=TRUE)
  
  d_fscores <- tibble(data_id = rownames(d_mat), 
                      ability = mirt::fscores(mod_2pl, method = "MAP")[,1])
  d_fscores$language <- language
  d_fscores$form <- form
...
write_feather(mirt_params, "data/vocabulary/mirt_params_2pl.feather")
```


```{r vocab-variability-IRT-slopes, fig.height=7, fig.cap="Standard deviation of latent ability scores from 4PL IRT models fit to each Words and Sentences-type dataset. Panels show individual languages. Smoothing lines are linear model fits."}
...
p100 %>%
  ...
  kable(caption = "Comparison of language milestones with walking. 25th and 75th give the ages at which that percentile ranking is achieved; ranges give the interval between 25th and 75th percentile in months and as a proportion of total age.", digits = 2)
```
...
In sum, our results echo the conclusions of @bornstein2005 based their comparative study of Spanish, English, and Italian. They noted that “individual variability is probably a universal feature of early language acquisition” (p. 311). 
