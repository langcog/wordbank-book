...
One study nicely illustrates why such an approach might not be misguided. @bornstein2012 collected multiple language measures at 20 and 48 months in a sample of nearly 200 children and used a structural equation model to estimate the stability of a single latent construct, language ability. Essentially all measures related strongly to this latent variable and the coefficient on its stability over time was _r_ = .84, suggesting that early language is quite stable, at least when measured appropriately. Notably, the ELI, a precursor to the CDI, was included in the measures at 20 months and was found to correlate with the 20-month latent construct at _r_ = .87.
...
```{r vocab-medians, fig.height=6, fig.cap="Median production using Words & Gestures-type forms. Included are only languages where there are more than 200 administrations total."}
lang_ns <- admins %>% 
  group_by(language, form) %>% 
...
```{r vocab-medians-production-ws, fig.height=6, fig.cap="Median production using Words & Sentences-type forms. Included are only languages where there are more than 200 administrations total."}
...
```{r vocab-medians-comprehension-wg, fig.height=6, fig.cap="Median comprehension using Words & Gestures-type forms. Included are only languages where there are more than 200 administrations total."}
ggplot(filter(medians, form %in% WGs, n_admins > 200), 
...
In the analyses above, our comparisons revealed two cases where datasets showed large disparities: Mandarin (Beijing) Words & Sentences production and Mandarin (Taiwan) Words & Sentences comprehension. 
...
ggplot(madm_summary_prod, aes(y = language, x = madm_mean, col = form)) + 
  ...
  # coord_flip() + 
  xlab("Language") + 
  ylab("MADM (production 12-24 months)") +
...
```{r vocab-variability-IRT-slopes, fig.height=7, fig.cap="Standard deviation of latent ability scores from IRT models fit to each Words & Sentences-type dataset. Panels show individual languages. Smoothing lines are linear model fits."}
