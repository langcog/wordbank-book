...
analysis_combos <- left_join(tibble(form = c("WG","WG","WS"), 
                                    ...
                                    measure_name = c("comprehension","production",
                                                     "production"),
                                    index = 1:3), 
                             expand.grid(index = 1:3, 
                                         demo = c(quo(sex), 
                                                  quo(birth_order), 
                                                  quo(mom_ed)))) %>%
  mutate(reference_level = 
           case_when(rlang::quo_get_expr(demo[[1]]) == "sex" ~ "Male", 
                     rlang::quo_get_expr(demo[[1]]) == "birth_order" ~ "Second",
                     rlang::quo_get_expr(demo[[1]]) == "mom_ed" ~ "Secondary"), 
         high_level =  
           case_when(rlang::quo_get_expr(demo[[1]]) == "sex" ~ "Female", 
                     rlang::quo_get_expr(demo[[1]]) == "birth_order" ~ "First",
                     rlang::quo_get_expr(demo[[1]]) == "mom_ed" ~ "College and Above"), 
...
```{r demo-gender-plot-wg-comp, fig.cap="Sex effects on comprehension scores plotted by age across languages."}
...
```{r demo-gender-mmad-wg-comp, fig.cap = "MMAD for female advantage, plotted by age and language, WG comprehension."}
...
```{r demo-gender-mmad-plot, fig.cap="Average MMAD for female advantage across ages, WG comprehension."}
...
```
Applying this measure to the data on comprehension, we see a quite small average female advantage that appears relatively constant across age (Figure \@ref(fig:demo-gender-mmad-wg-comp)). For those languages with dense enough data, we can take a weighted average of this pattern across ages, which reveals substantial variability (Figure \@ref(fig:demo-gender-mmad-plot)). The overall median for these `r nrow(analyses$WG.comprehension$sex$mmad_summary)` languages is quite small as well, `r round(median(analyses$WG.comprehension$sex$mmad_summary$mmad_mean),2)`. In summary, there is some evidence for a modest female advantage in comprehension. 
