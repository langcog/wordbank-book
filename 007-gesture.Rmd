...
```{r gesture-first-gesture-figure, fig.width = 8.5, fig.height = 6, fig.cap="FIXME"}
...
For comparison, we collapse the "sometimes" and "often" into a single value, and plot the proportion of children at each age whose parents report that they produce each gesture (Figure \@ref(fig:gesture-first-gesture-binary-figure)). The trajectories look generally smooth and prima facie reasonable, with the potential exception of the "smacklip" gesture for which there is very little developmental change (this gesture, which corresponds to the vocalization "yum yum," may be unusual or less stereotyped).
...
```{r gesture-first-gesture-binary-figure, fig.width = 6, fig.height = 4, fig.cap="FIXME"}
...
```{r gesture-other-figure, fig.width = 8, fig.height = 7, fig.cap="FIXME"}
ggplot(other_gestures_ordered, aes(x = age, y = n, color = definition, 
                                  label = definition)) +
  facet_wrap(~ type, labeller = label_caps) + 
  geom_point() + 
  geom_smooth(method = "loess", se = FALSE) +
  directlabels::geom_dl(method = list(directlabels::dl.trans(x=x -.2),
                                      "first.qp", cex=0.8, fontfamily = .font)) +
...
```{r gesture-aoa, fig.width = 6, fig.height = 4, fig.cap="FIXME"}
...
kable(mean_crossling_aoas, digits = 2, caption = "FIXME",
      col.names = c("Item", "Mean", "SD", "N"))
```

Table \@ref(tab:gesture-crossling-first) and Figure \@ref(fig:gesture-crossling-fig) show both consistency and variability across items. As in the learning of words, the means and variances of these ages of acquisition were correlated [_r_ = `r signif(cor(mean_crossling_aoas$mean, mean_crossling_aoas$sd), digits = 2)`; @mollica2017]. The primary outliers were "request", which appears to be produced surprisingly late in American English, and "shrug" which was produced surprisingly late by French-learning infants. In general, however, most of the cross-linguistic differences appear to be consistent across the gestures (i.e. French-learning infants gestures later). It is difficult to tell from this small sample of mostly European languages whether these differences are driven by linguistic factors or rather by properties of our samples or variability in parents' interpretation of the form. Nonetheless, they provide some evidence for consistency in the process of gestural development cross-linguistically. To get additional leverage on this process, we next consider the full set of gestures.

```{r gesture-crossling-fig, fig.width = 8, fig.height = 5, fig.cap="FIXME"}
crossling_aoas %>%
  ungroup() %>%
...
kable(age_cors, digits=3, caption = "FIXME",
...
kable(gest_lang_data, digits = 3, caption = "FIXME",
