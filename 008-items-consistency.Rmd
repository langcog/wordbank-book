...
highlight_produces <- top10 %>%
  filter(measure == "produces") %>%
  pivot_longer(cols = Croatian:Turkish, names_to = "language", values_to = "word") %>%
  group_by(word) %>%
  summarise(n = n()) %>%
  filter(n >= MIN_HIGHLIGHT_PRODUCTION) %>%
  mutate(bold_word = paste0("**",word,"**"))

top10 %>%
  ungroup() %>%
  filter(measure == "produces") %>%
  mutate_all(function(col) str_remove(col, " \\(.*\\)")) %>%
  pivot_longer(cols = c(-measure, -order), names_to = "language", values_to = "word") %>%
  left_join(select(highlight_produces, -n), by = "word") %>%
  mutate(word = if_else(is.na(bold_word), word, bold_word)) %>%
  select(-bold_word) %>%
  pivot_wider(names_from = "language", values_from = "word") %>%
  select(-measure, -order) %>%
  kable(caption = paste0("The 10 earliest words that children produce in each language. Bolded words appear in at least ", MIN_HIGHLIGHT_PRODUCTION, " languages.")) %>%
...
highlight_understands<- top10 %>%
  filter(measure == "understands") %>%
  pivot_longer(cols = Croatian:Turkish, names_to = "language", values_to = "word") %>%
  group_by(word) %>%
  summarise(n = n()) %>%
  filter(n >= MIN_HIGHLIGHT_COMPREHENSION) %>%
  mutate(bold_word = paste0("**",word,"**"))
...
top10 %>%
  ...
  mutate_all(function(col) str_remove(col, " \\(.*\\)")) %>%
  pivot_longer(cols = c(-measure, -order), names_to = "language", values_to = "word") %>%
  left_join(select(highlight_understands, -n), by = "word") %>%
  mutate(word = if_else(is.na(bold_word), word, bold_word)) %>%
  select(-bold_word) %>%
  pivot_wider(names_from = "language", values_from = "word") %>%
  select(-measure, -order) %>%
   kable(caption = paste0("The 10 earliest words that children understand in each language. Bolded words appear in at least ", MIN_HIGHLIGHT_COMPREHENSION, " languages.")) %>%
  kableExtra::kable_styling(font_size = 10.5)
```
