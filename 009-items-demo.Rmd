...
---
# Demographic Variation in Individual Words^[An earlier version of the gender analyses below was presented to the Boston University Conference on Language Development in 2016] {#words-demographics}
...
```{r 9_props_plot, dependson="9_plot_props", include=FALSE}
...
```{r 9_coefs_plot_bo, dependson="9_plot_coefs", fig.height=4, eval=FALSE}
plot_demo_coefs("birth_order")
```
```{r 9_coefs_plots_me, dependson="9_plot_coefs", fig.height=4, eval=FALSE}
plot_demo_coefs("mom_ed")
```
```{r 9_coefs_plots_sex, dependson="9_plot_coefs", fig.height=6, fig.width=8, eval=FALSE}
plot_demo_coefs("sex")
```
...
```{r items_demo_sex_table_understands}
demo_ranef %>%
...
```{r items_demo_sex_table_produces}
demo_ranef %>%
  filter(!is.na(sexMale), 
         measure == "produces", 
         abs(sexMale) > .5) %>%
...
```{r items_demo_bo_table_understands}
demo_ranef %>%
...
```{r items_demo_bo_table_produces}
demo_ranef %>%
  filter(!is.na(birth_order1), !is.na(birth_order2),
         measure == "produces", 
         abs(birth_order1) > .5 | abs(birth_order2) > .5) %>%
...
```{r 9_ranef_plots_me_comp, dependson="9_plot_ranef"}
plot_demo_ranef("mom_ed", "understands")
```

```{r items_demo_momed_table_understands}
demo_ranef %>%
...
```{r 9_ranef_plot_me_prod, dependson="9_plot_ranef"}
plot_demo_ranef("mom_ed", "produces")
```

```{r items_demo_momed_table_produces}
demo_ranef %>%
  filter(!is.na(mom_ed1), !is.na(mom_ed2),
         measure == "produces", 
         abs(mom_ed1) > .5 | abs(mom_ed2) > .5) %>%
