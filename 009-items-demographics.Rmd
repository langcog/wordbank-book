...
```{r itemsdemo-sex-table-comp, results="asis"}
cat(dt_caption(glue::glue("Sex item random effects with magnitude at least {tab_thres} for comprehension data in each language.")))
demo_ranef %>%
  filter(!is.na(sexMale), 
         measure == "understands", 
         abs(sexMale) > tab_thres) %>%
  mutate(male_advantage = signif(sexMale, 2)) %>%
  select(language, item = definition, male_advantage) %>%
...
```{r itemsdemo-sex-table-prod, results="asis"}
cat(dt_caption(glue::glue("Sex-based item random effects with magnitude at least {tab_thres} for production data in each language.")))
demo_ranef %>%
  filter(!is.na(sexMale), 
         measure == "produces", 
         abs(sexMale) > tab_thres) %>%
