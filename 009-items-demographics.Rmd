...
plot_demo_fixed <- function(dem) {
  ...
  ggplot(plot_coefs, aes(x = estimate, y = group, colour = term)) +
    ...
    scale_colour_manual(values = term_colours, name = dem) +
    guides(colour = guide_legend(nrow = 1)) +
    labs(x = "Odds ratio (fixed effect)", y = "") +
    theme(legend.position = "top", legend.direction = "vertical", legend.justification = c(0, 0),
...
tops <- demo_ranef_coded %>%
  ...
  separate(term, c("numerator", "denominator"), sep = " : ", remove = FALSE) %>%
...
top_plot <- function(dem, meas, nc = 4) {
  tops %>%
    ungroup() %>%
    ...
    mutate(definition = str_remove(definition, " \\(.*\\)$"),
           direction = direction %>%
             ...
             fct_recode(!!!dir_codes[[dem]])) %>%
    ggplot(aes(x = effect, y = rank, colour = term)) +
      facet_wrap(~language + direction, scales = "free", ncol = nc) +
...
```{r itemsdemo-tops-sex-comp, dependson="itemsdemo-tops_ranef", fig.height=18, fig.cap=glue::glue("Top {num_tops} most sex biased words in each language for comprehension data.")}
top_plot("sex", "understands")
```

Figures \@ref(fig:itemsdemo-ranef-plot-sex-prod) and \@ref(fig:itemsdemo-tops-sex-prod) give the same measures for production. There are considerably more words per language with large gender biases (at least `r exp(threshold)` times higher in either direction) for production (M = `r 100*signif(threshold_prop("produces", "sex"), 3)`%) than for comprehension (M = `r 100*signif(threshold_prop("understands", "sex"), 3)`%). The content of these gender biases is extremely similar across language. For English, we see the largest biases in each direction for genital terms, male biases for vehicles and objects associated with traditionally male activities (e.g., sports), and a female bias for female-coded clothing and toys. This pattern is replicated quite robustly across languages, although with varying magnitudes.
...
```{r itemsdemo-tops-sex-prod, dependson="itemsdemo-tops_ranef", fig.height=26, fig.cap=glue::glue("Top {num_tops} most sex biased words in each language for production data.")}
...
```{r itemsdemo-tops-bo-comp, dependson="itemsdemo-tops_ranef", fig.height=8, fig.cap=glue::glue("Top {num_tops} most sex biased words in each language for comprehension data.")}
top_plot("birth_order", "understands", 2)
```

The same general patterns are present in the production data (Figures \@ref(fig:itemsdemo-ranef-plot-bo-prod) and \@ref(fig:itemsdemo-tops-bo-prod)), with further evidence that having elder siblings appears to be related exposure to sweets, at least in some cultures: _popsicle_, _donut_, and _candy_ all appear now in the English data, and _tyggegummi_ (gum) and several soda- and candy-related words appear in the Norwegian data. _Hate_ also appears in the English data, suggesting some emotional expressions due to having a sibling. We interpret this pattern with caution, however, as birth order is likely partially confounded with socioeconomic status, and so later-born children might also be from low-SES families who have more environmental exposure to "junk foods" like soda and candy.
...
```{r itemsdemo-tops-bo-prod, dependson="itemsdemo-tops_ranef", fig.height=14, fig.cap=glue::glue("Top {num_tops} most birth order biased words in each language for production data.")}
top_plot("birth_order", "produces", 2)
```
...
```{r itemsdemo-plot-pronouns, fig.height=1.5, fig.cap="Histogram showing item random effects for personal pronouns by birth order, across languages. Red line marks zero, blue line marks the mean, blue band shows a 95\\% confidence interval."}
...
```{r itemsdemo-tops-me-prod, dependson="itemsdemo-tops_ranef", fig.height=12, fig.cap=glue::glue("Top {num_tops} most maternal education biased words in each language for production data.")}
