...
We limit our analysis to traditional WS and WG forms (along with variants in these classes) because short forms like the British English TEDS don't typically include category information. The sample sizes included in this analysis are given below. 
...
ggplot(area_scatter_data, 
  ...
  ggrepel::geom_label_repel(aes(label=langform), force = 4, alpha = .8, size = 3) +
  xlab("Noun bias") +
  ylab("Predicate bias") + 
  xlim(-.05,.15) + 
  ylim(-.2,.1)
...
* Every language showed a substantial bias against function words, supporting the generalization that these are acquired much later than content words, despite their typically higher frequency (see \@ref(items-prediction)). This bias was larger on average than biases in type of content words. 
