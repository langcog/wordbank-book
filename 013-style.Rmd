...
## Variation in Vocabulary Growth Rate

Another potential locus for individual differences is the rate of growth. As already discussed in Chapter \@ref(vocab), there is substantial variability between children in vocabulary size. As we showed there, across many languages there is a consistent -- and high -- rate of variability between children. 
...
```
Sampling only those children who were measured a large number of times (>10, Norwegian only due to data sparsity issues in English), the trajectories appear quite stable. 
...
ggplot(filter(n_long_ws, n_admins > 10, 
  ...
  scale_colour_discrete(guide = FALSE)
```
We can see that these are visually quite stable. We can quantify this by the standard deviation.

```{r}
n_long_ws %>%
  group_by(language, original_id) %>%
  summarise(sd = sd(percentile, na.rm = TRUE), 
            mean_pairwise_diff = mean(diff(percentile), na.rm=TRUE)) %>%
  summarise(sd = mean(sd, na.rm = TRUE), 
            mean_pairwise_diff = mean(mean_pairwise_diff, na.rm=TRUE))
```

Another way to look at this is the difference in percentile ranks by the age gap.

```{r}
ms <- n_long_ws %>%
  group_by(language, original_id) %>%
  mutate(pct_diff = c(NA, diff(percentile)), 
         age_diff = c(NA, diff(age))) 

ggplot(ms, aes(x = age_diff, y= pct_diff)) + 
  geom_jitter(alpha = .1) + 
  geom_smooth(method = "lm") + 
  facet_wrap(~language)+ 
...
range <- n_long_wg %>%
  group_by(original_id) %>%
  arrange(age) %>%
  mutate(dv = c(NA, diff(production)), 
         dt = c(NA, diff(age))) %>%
  filter(!is.na(age))

ggplot(filter(range, n_admins > ADMIN_CUTOFF), 
       aes(x = age, y = dv)) + 
  # geom_line() + 
  geom_smooth(span = 1, col = "blue", se = FALSE) + 
  geom_point() + 
  facet_wrap(~fct_reorder(original_id, production)) +
  scale_colour_discrete(guide = FALSE) + 
  # scale_x_continuous(breaks = c(8, 16)) + 
...
ggplot(filter(range, n_admins > ADMIN_CUTOFF), 
  ...
  scale_colour_discrete(guide = FALSE) + 
  # scale_x_continuous(breaks = c(8, 16)) + 
  scale_y_continuous(breaks = c(-100, 50, 150), lim = c(-100,200)) +
  theme(strip.text = element_blank())
```
