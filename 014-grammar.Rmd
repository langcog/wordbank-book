...
grammar_fits_grouped %>%
  filter(language == "French (French)", coded_type == "Complexity") %>% View()
  ggplot(aes(x = production_prop, y = mean_fitted)) +
    geom_point()
...
We then fit logistic regression models with linear, quadratic, and cubic predictors (as above) separately for every item. Figure \@ref(fig:gram-item-effects-plot) shows the age effect coefficient of each item. In general, age effect were smaller for Word Form items, then Morphological Complexity items, and largest for Syntactic Complexity items, suggesting that more syntactic phenomena likely have greater age contributions. 
