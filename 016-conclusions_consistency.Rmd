...
cvs <- data_frame(name = files) %>%
  ...
  mutate(y = 0, 
         signature = fct_reorder(signature, cv), 
...
For the sake of our analysis, we have broken phenomena from the preceding chapters into four categories. These are 

```{r cons-cvs_plot_1}
ggplot(cvs, aes(x = signature, y = cv, col = measure)) + 
