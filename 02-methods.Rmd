...
We begin by introducing our dataset and the database that contains it. We then discuss the subsidiary variables and analytic techniques used throughout the book. Where appropriate, we defer details of particular statistical techniques to specific chapters. Our starting place is the basic structure of CDI data. Some material in this chapter is adapted from @frank2016.
...
As mentioned above, datasets come from a variety of sources. In all cases, the preferred citation for each dataset and its contributor is given on the [Wordbank contributors page](http://wordbank.stanford.edu/contributors.html). Several of these datasets were transferred second-hand from a pre-existing database (CLEX-CDI; @jorgensen2010), while many of the others were contributed directly via electronic or paper forms. 

In the case of paper forms, we rekeyed the forms using double-entry methods (either ourselves or via a commercial contractor).^[In one check for errors in the rekeying of Korean data, we found that there were 4 incorrect fields in 10 records
...
ggplot(gender_counts, 
       aes(x = language, y = prop, fill = sex)) + 
...
ggplot(ord_counts, 
  ...
  xlab("") +
  scale_color_solarized(name = "Birth Order") 
```
