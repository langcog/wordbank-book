...
madm_data <- vocab_data %>%
  ...
  summarise(madm = MADM(production), 
            n = n()) %>%
  filter(n > 20) 

ggplot(filter(madm_data, language == "English (American)"), 
       aes(x = age, y = madm, col = form)) + 
  geom_point() + 
...
```{r 3_madm2}
ggplot(madm_data, aes(x = age, y = madm, col = language)) + 
...
madm_summary <- madm_data %>%
  ...
  summarise(madm_mean = mean(madm), 
            madm_ci_lower = mean(madm) - ci.t(madm),
            madm_ci_upper = mean(madm) + ci.t(madm)) 
...
ggplot(comp_data,
  ...
  ylab("Comprehension Vocabulary (proportion)") +
  ylim(c(0, 1)) + 
  theme(legend.position = "bottom")
```
