...
get_demo_preds <- function (models, data, demo = ~sex) {
  ...
  demo_name <- as.character(demo)[2] # fixme - how do I coerce this more nicely?
...
analysis_combos <- left_join(tibble(form = c("WG","WG","WS"), 
  ...
  mutate(reference_level = case_when(demo == quo(sex) ~ "Male", 
         ...
         demo_name = str_replace(as.character(demo), "~", ""))
  ...
  map(function (z) {
    z %>%
      ...
      map(function(x) {
        ...
        demo <- x$demo[1][[1]]
        measure <- x$measure[1][[1]]
        
        print(demo)
        print(measure)
        
        data <- vocab_data %>%
          filter(!is.na(!!demo), form == x$form[1])
        ...
        results$model_summary <- get_demo_model_summary(results$models, level = x$reference_level[1])
        results$mmad_ratio <- get_mmad_ratio(data, measure = measure, demo = demo, 
                                         high_level = x$high_level[1], low_level = x$reference_level[1])
