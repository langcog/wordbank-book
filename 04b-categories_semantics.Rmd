...
bp_means <- bp %>%
  ...
  group_by(language, form, uni_lemma, age, measure) %>%
...
upper_lower <- filter(bp_means, measure == "understands", n > 10, language != "Korean",  
              form == "WG", uni_lemma %in% body_unis) %>%
...
```{r}
face <- filter(bp_means, measure == "understands", n > 10, language != "Korean",  
              form == "WG", uni_lemma %in% face_unis) 
# %>%
  # mutate(body_position = ifelse(uni_lemma %in% c("arm","hand","finger"), "upper", "lower"), 
  #        size = case_when(uni_lemma %in% c("arm","leg") ~ "arm/leg", 
  #                         uni_lemma %in% c("hand","foot") ~ "hand/foot",
