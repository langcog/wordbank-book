...
kid_wg_data <- wg_data %>%
  ...
  dplyr::select(data_id, value, age, language, uni_lemma, category, lexical_category, lexical_class)
...
stitched_data <- ws_sub_data %>%
  ...
  left_join(admins) %>%
  filter(!is.na(age)) %>%
  dplyr::select(data_id, value, age, language, uni_lemma, category, 
...
To begin, we ask about the earliest words. Following @tardiff2008, we present the first 10 words acquired by children across langauges
...
top10 <- all_aoas %>%
  ...
  arrange(rglm_aoa) %>%
  slice(1:10) %>%
  dplyr::select(-rglm_aoa, -lexical_category) %>%
  mutate(order = 1:n()) %>%
...
aoa_corr <- function(df, lang1, lang2) {
 ...
 corrs <- df %>%
    dplyr::select(-lexical_category, -measure) %>%
...
production_hc <- hclust(dist(dplyr::select(production_wide, -lang1)))
...
understanding_hc <- hclust(dist(dplyr::select(understanding_wide, -lang1)))
...
if(!local_data) {
  ...
  write_feather(understands_step_corrs, "data/understands_step_corrs.feather")
  write_feather(produces_step_corrs, "data/produces_step_corrs.feather")

} else{
  understands_step_corrs <- read_feather("data/understands_step_corrs.feather")
  produces_step_corrs <- read_feather("data/produces_step_corrs.feather")
}
