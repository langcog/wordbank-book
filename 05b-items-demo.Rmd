...
raw_data <- items %>%
  filter(form == "WS", language %in% c("Croatian","Czech")) %>%
  split(.$language) %>%
  map(get_lang_data)

write_feather(bind_rows(raw_data), "data/gender_items.feather")
```


```{r gender_read}
raw_data <- read_feather("data/gender_items.feather") %>%
  split(.$language)
```
...
fit_gender_diffs <- function(lang_data) {
  ...
  gender_props <- lang_data %>%
    filter(!is.na(sex)) %>%
    group_by(language, form, sex, age, lexical_class, category,
  ...
  gender_fits <- gender_props %>%
    group_by(language, form, sex, lexical_class, category,
             uni_lemma, definition) %>%
    nest() %>%
...
gender_diffs <- raw_data %>% 
  map_df(fit_gender_diffs)
```
