...
get_grammar_data <- function(lang) {
  ...
  lang_admins <- admins %>%
    ...
    left_join(complexity_forms %>% select(language, word) %>% rename(num_words = word)) %>%
...
ggplot(filter(grammar_summary, 
              !is.na(language), 
              !(language %in% c("Slovak","Mandarin"))),
...
ggplot(filter(grammar_summary_age, 
              !is.na(language), 
              !(language %in% c("Slovak","Mandarin"))),
