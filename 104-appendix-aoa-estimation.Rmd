...
```{r appaoa-read_data, eval=FALSE}
eng_ws <- get_instrument_data(instrument_language = "English (American)",
                           instrument_form = "WS", 
                           iteminfo = TRUE,
                           administrations = TRUE)
write_feather(eng_ws, "data/eng_ws_raw_data.feather")
```
...
ggplot(aoas, aes(x = bglm_aoa, y = glm_aoa)) + 
  geom_point() + 
  geom_label_repel(data = filter(aoas, abs(bglm_aoa - glm_aoa) > 1.5), 
...
save(samps, "data/aoa-samps.Rds")
```

Diagnostics.

```{r appaoa-stan-diagnostics, eval=FALSE}
load("data/aoa-samps.Rds")
