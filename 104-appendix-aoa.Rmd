...
```{r appaoa-compare-dist, fig.cap="Histogram of English (American) age of acquisition values as estimated via a variety of statistical methods (panels)."}
...
aoas <- left_join(bglm_aoas_sparse, 
                  bglm_aoas) %>%
  left_join(glm_aoas_sparse)

a <- ggplot(aoas, aes(x = bglm_aoa, y = glm_aoa_sparse)) +
  geom_point() +
  ggrepel::geom_label_repel(data = filter(aoas, abs(bglm_aoa - glm_aoa_sparse) > 4),
                   aes(label = definition)) +
  ylab("GLM (sparse data)") +
  xlab("Bayes GLM (full dataset)") +
  geom_abline(lty = 2, col = "red")

b <- ggplot(aoas, aes(x = bglm_aoa, y = bglm_aoa_sparse)) +
  geom_point() +
  ggrepel::geom_label_repel(data = filter(aoas, abs(bglm_aoa - bglm_aoa_sparse) > 4),
                   aes(label = definition)) +
  ylab("Bayes GLM (sparse data)") +
  xlab("Bayes GLM (full dataset)") +
  geom_abline(lty = 2, col = "red")
