...
#   spread(type, n) %>%
#   dplyr::select_(.dots = c("language", "word", morphosyntax_fields)) #%>%
# 
...
get_grammar_data <- function(lang) {
  ...
  lang_admins <- admins %>%
    ...
    dplyr::select(language, form, age, data_id, production) %>%
  ...
  lang_grammar_data %>%
    ...
    dplyr::select(language, age, data_id, vocab_mean, item_id, definition, type,
...
```{r 7_grammar_data_plot_complexity}
ggplot(filter(grammar_summary, coded_type == "Complexity (Syntactic)", language != "Slovak"),
       aes(x = vocab_mean, y = mean)) + 
  geom_jitter(alpha = 0.2, size = 0.75) +
  facet_wrap(~ language) + 
  geom_smooth(span = 1) +
...
Define our signature here as $r^2$.

```{r}
r2 <- function(x) {
    r2 <- summary(lm(mean ~ vocab_mean, data = x))$r.squared
    d <- data_frame(language = x$language[1], 
                      coded_type = x$coded_type[1], 
                      r2 = r2)
    return(d)
    }

r2s <- filter(grammar_summary, coded_type != "Combine") %>%
  group_by(language, coded_type) %>%
  do(r2(.))

cvs <- r2s %>%
  filter(is.finite(r2)) %>%
  group_by(coded_type) %>%
  summarise(cv = sd(r2) / mean(r2)) %>%
  mutate(foo = "and Vocab (r2)") %>%
  unite(measure, coded_type, foo)
...
kable(dplyr::select(grammar.model.metrics,
